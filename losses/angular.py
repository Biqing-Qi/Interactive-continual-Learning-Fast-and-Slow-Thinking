...
class BlendedLoss(object):
    def __init__(self, main_loss_type, cross_entropy_flag):
        ...
        assert main_loss_type in MAIN_LOSS_CHOICES, (
            "invalid main loss: %s" % main_loss_type
        )
    ...
    def calculate_loss(self, target, output_embedding, output_cross_entropy=None):
        ...
        if self.cross_entropy_flag:
            assert (
                output_cross_entropy is not None
            ), "Outputs for cross entropy loss is needed"
        ...
        main_loss = (
            main_loss_outputs[0]
            if type(main_loss_outputs) in (tuple, list)
            else main_loss_outputs
        )
...
class NPairLoss(nn.Module):
    ...
    def n_pair_loss(anchors, positives, negatives):
        ...
        x = torch.matmul(
            anchors, (negatives - positives).transpose(1, 2)
        )  # (n, 1, n-1)
...
class AngularLoss(NPairLoss):
    ...
    def __init__(self, l2_reg=0.02, angle_bound=1.0, lambda_ang=2, **kwargs):
    ...
    def forward(self, embeddings, target):
        ...
        losses = self.angular_loss(
            anchors, positives, negatives, self.angle_bound
        ) + self.l2_reg * self.l2_loss(anchors, positives)
    ...
    def angular_loss(anchors, positives, negatives, angle_bound=1.0):
        ...
        x = 4.0 * angle_bound * torch.matmul(
            (anchors + positives), negatives.transpose(1, 2)
        ) - 2.0 * (1.0 + angle_bound) * torch.matmul(
            anchors, positives.transpose(1, 2)
        )  # (n, 1, n-1)
...
class NPairAngularLoss(AngularLoss):
    ...
    def forward(self, embeddings, target):
        ...
        losses = self.n_pair_angular_loss(
            anchors, positives, negatives, self.angle_bound
        ) + self.l2_reg * self.l2_loss(anchors, positives)
