        ...
        for i in range(n):
            ...
            if self.hard_mining is not None:
                neg_pair = torch.masked_select(
                    neg_pair_, neg_pair_ + 0.1 > pos_pair_[0]
                )
                pos_pair = torch.masked_select(
                    pos_pair_, pos_pair_ - 0.1 < neg_pair_[-1]
                )
                ...
                pos_loss = (
                    2.0
                    / self.beta
                    * torch.log(torch.sum(torch.exp(-self.beta * pos_pair)))
                )
                neg_loss = (
                    2.0
                    / self.alpha
                    * torch.log(torch.sum(torch.exp(self.alpha * neg_pair)))
                )
